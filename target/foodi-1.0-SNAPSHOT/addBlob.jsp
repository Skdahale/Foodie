<%-- 
    Document   : addBlob
    Created on : May 23, 2023, 2:25:57 AM
    Author     : 91860
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>

<html>
    <head>
    </head>
    <body>
        <div class="container">
            <div class="row">
                <div class="col" style="display: flex; justify-content: center; align-content: center">
                    <div class="main-details-div">
                        <form action="BLOB-redirect" method="post" style="font-family: 'Poppins', sans-serif;" id="form" enctype="multipart/form-data">
                            <div class="row">
                                <div class="col">
                                    <br><div class="details-col col col2">
                                        <br><h4 style="font-weight: 700; margin-left: 20px"> Images</h4>
                                        <p style="color: grey; margin-left: 40px; font-size: 0.9em">Upload various pictures of your outlet</p>
                                        <br>
                                        <input id="images" type="file" style="margin-left: 40px; padding: 10px;" accept="image/png, image/jpeg" name="s"/>
                                        <br><br><br>
                                    </div>
                                </div>
                            </div>
                            <br><br><input type="submit" name="submit-button" id="submit-btn" value="   Submit   " class="submit-btn"/>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </body>
</html>
